...
if($request == 'POST'){
    ...
    $urlDecode = urldecode($json);
    parse_str($urlDecode, $objectURL);
    //inclusão de resultado em debug
    $text .= ''.PHP_EOL;
    $text .= ''.PHP_EOL;
    $text .= 'POST RECEIVED - '.date("Y-m-d H:i:s").PHP_EOL;
    //$text .= $urlDecode.PHP_EOL;
    //$text .= $objectURL["event"].PHP_EOL;
    //$text .= $objectURL["data"]["FIELDS"]["ID"].PHP_EOL;
    
    //RECEBENDO INTEGRAÇÃO CLICKSIGN
    if(isset($resultado["event"]["name"])){
        ...
        if($event === "auto_close" and $status === "closed" ){
          $text .= 'Tratativa para o evento iniciada'.PHP_EOL;
            ...
            $vencimento = date("Y-m-d",strtotime(date("Y-m-d").' +4 days'));
              ...
              if($fatura_id > 0){
                $return_fatura = updateDateFatura($fatura_id,$vencimento);
                if(isset($return_fatura) and $return_fatura > 0){
                  $text .= 'Vencimento de fatura atualizado '.$return_fatura.PHP_EOL;
                }else{
                  $text .= 'Vencimento de fatura não atualizado '.$return_fatura.PHP_EOL;
                }
                ...
                $text .= 'Não foi localizado o ID da fatura '.$fatura_id.PHP_EOL;
                
                  header('Content-Type: application/json');
                  $retorno = array("Deal" => $deal_id, "Document Key" => $document, "Event" => $event);
                  $json_send = json_encode(array("error" => "false", "result" => $retorno, "message" => "Evento processado, fatura principal não foi gerada"),true); 
                  echo $json_send;
              }
    ...
    }else if(isset($resultado["leads"][0]["first_conversion"])){
    ...
    }else if(isset($objectURL)){
      //RECEBENDO DADOS VIA INTEGRAÇÃO BITRIX24 OUTBOUND WEBHOOK
      if(isset($objectURL["event"])){
        $text .= "RECEIVED EVENT BITRIX24".PHP_EOL;
        $text .= "Evento Recebido: ".$objectURL["event"].PHP_EOL;
        $text .= "ID Entidade Recebida: ".$objectURL["data"]["FIELDS"]["ID"].PHP_EOL;

        //IMPLEMENTAR AQUI A LISTA DE METHODS QUE SERÃO RECEBIDOS DO BITRIX
        //
        //
        //
        //
        ...
        header('Content-Type: application/json');
        echo json_encode(array("error" => "false", "erro_msg" => "Event ".$objectURL["event"]." received."), true);

      }else{
        $text .= "Event description no localizable".PHP_EOL;

        header('Content-Type: application/json');
        echo json_encode(array("error" => "true", "erro_msg" => "Parameter EVENT not received"), true);

      }
      
    }else{
      header('Content-Type: application/json');
      echo json_encode(array("error" => "true", "erro_msg" => "Method of integration not avaliable"), true);
    }
...
function updateDateFatura($fatura_id,$vencimento){
  $fatura='';
  if(isset($fatura_id))
      {
        ...
        $payloaGetInvoice ='{
          "ID":'.$fatura_id.',
          "fields": {
            "DATE_PAY_BEFORE": "'.$vencimento.'"
          }
        }';
        
        $updDeal = json_decode(curl_bitrix_method("crm.invoice.update",$payloaGetInvoice),true);
