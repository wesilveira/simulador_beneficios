<?php
	...
	switch($tipoForm){
		case "declaracao":
			...
			$perguntas = $_POST["numq"];
			$id_form = (isset($_POST["id_form"])) ? $_POST["id_form"] : false;
			...
			for ($i = 1; $i <= $perguntas; $i++) {
				...
				if(isset($_POST["radio_tit_$i"])){
					$linhas_respostas .= '<td>R'.$i.'</td><td align="center">'.$_POST["radio_tit_$i"].'</td>';
				}else{
					$linhas_respostas .= '<td>R'.$i.'</td><td align="center">nao</td>';

				}

				if(isset($_POST["radio_tit_$i"]) and $_POST["radio_tit_$i"] == "sim")
				{
					$qtobs++;
					...
					if($id_form != "ds")
					{
						...
						$data_tit = "&fields[".$campos_depara_bitrix["OBS".$qtobs."_data_evento"]."]=".$_POST["data_obs_tit_$i"];
				...
				if($dependentes > 0){
					for ($dep = 1; $dep <= $dependentes; $dep++) {
						...
						if(isset($_POST[$dep_item])){
							$linhas_respostas .= '<td align="center">'.$_POST[$dep_item].'</td>';
						}else{
							$linhas_respostas .= '<td align="center">nao</td>';
						}
						
						//campos bitrix variaveis
						$campo_radio_bitrix_dependente = "radio_".$i."_DEP0".$dep;
						
						if(isset($_POST[$dep_item]) and $_POST[$dep_item] == "sim")
						{
							$qtobs++;
							...
							if($id_form != "ds")
							{
								...
								$data_dep = "&fields[".$campos_depara_bitrix["OBS".$qtobs."_data_evento"]."]=".$_POST[$data_obs_dep_item];
			...
			if($opcao_orientacao == 5804){
				$opcao_aceite .= "<b>Opção Orientação Médica</b>: <span>Declaro que fui orientado por médico referenciado da UNIMED no Preenchimento da Declaração de Saúde acima.</span><br>";
			}else if($opcao_orientacao == 5806){
				$opcao_aceite .= "<b>Opção Orientação Médica</b>: <span>Declaro que fui orientado pelo meu médico Particular no Preenchimento da Declaração de Saúde acima.</span><br>";
			}else if($opcao_orientacao == 5808){
				$opcao_aceite .= "<b>Opção Orientação Médica</b>: <span>Declaro ter conhecimento de todas as perguntas da Declaração de Saúde acima e que dispensei orientação médica para responder às mesmas assumindo total responsabilidade pelas informações por mim prestadas.</span><br>";
			}else if($opcao_orientacao == 3690){
	...
	//send to bitrix
	$base_url_deal_update = "https://heaolu.bitrix24.com.br/rest/4/dyelyvhk946iohux/crm.deal.update?";
	...
	$url_timeline_comment = "https://heaolu.bitrix24.com.br/rest/4/dyelyvhk946iohux/crm.timeline.comment.add?fields[ENTITY_ID]=".$_POST["deal_id"]."&fields[ENTITY_TYPE]=deal&fields[COMMENT]=".$timeline_comment;
	$result_send = json_decode(curl_rest($base_url_deal_update.$deal_id.$campos_bitrix_update));
		
	if(isset($result_send->result) and $result_send->result === true){	
		//Seta mensagem em timeline quando DS é preenchida
		curl_rest($url_timeline_comment);
		
		//Seta campo para receber status de DS preenchida (campo 0=sim, 1=não)
		$campos_bitrix_confirm_ds = "&fields[UF_CRM_1607005403]=1";
		$result_send_ds = json_decode(curl_rest($base_url_deal_update.$deal_id.$campos_bitrix_confirm_ds));
		echo '<script>alert("Declaração de saúde enviada com sucesso");</script>';
	}else{
		$errorMsg = ($result_send->error == "expired_token") ? "'[Token Expirado]'" : "Erro, verifique e tente novamente.".$result_send->error;
